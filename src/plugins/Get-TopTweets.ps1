...
$SearchedTweets = python.exe  ./src/plugins/Twitter.py $Search $configs['consumer_key'] $configs['consumer_secret'] $configs['token'] $configs['token_secret']
...
| Where-Object { (-not $_.retweeted_status) -and $_.date -gt (get-date).AddDays(-8) } `
