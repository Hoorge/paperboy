...
1..$pages | ForEach-Object {
    $xml = [xml] ((Invoke-WebRequest "https://blogs.msdn.microsoft.com/$Blog/feed/?paged=1").content)
    if($xml){
...
$article | Where-Object date -gt (Get-Date).AddDays(-7) | Select-Object -First $Count | ft -AutoSize   
